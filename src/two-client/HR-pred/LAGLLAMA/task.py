...
from embeddedexample.preprocess import preprocessing, process_with_Lag_llama, SmartcareProcess_with_Lag_llama
import os
import numpy as np
import glob
import re
...
def load_data_from_disk_one_patient_per_round_lagLLama(patient_count: int, DATASET_DIR: str):
    # Get sorted list of patient folders
    all_folders = sorted(glob.glob(os.path.join(DATASET_DIR, "user_*")), key=natural_sort_key)
    ...
    # Select the folder corresponding to the patient
    selected_folder = all_folders[patient_count - 1]  # 1-based indexing
    ...
    # Load and preprocess data
    df_rr = process_with_Lag_llama(filepath)

    # Print dataset info
    print(f"Loaded data from {selected_folder}.")
    print(f"Size of dataset: {len(df_rr)}")

    return df_rr



def load_SmartCare_data_from_disk_one_patient_per_round_lagLLama(patient_count: int, DATASET_DIR: str):
    ...
    # Load and preprocess the selected file
    df = SmartcareProcess_with_Lag_llama(selected_file)
...
def test(net, testloader, loss_fn, device, scaler):
    ...
    with torch.no_grad():
        for features, labels in testloader:
            ...
            loss = loss_fn(torch.tensor(yhat_original), torch.tensor(labels_original)).item()
            total_loss += loss
...
def train(net, trainloader, testloader, epochs, learning_rate, device, scaler):
    ...
    test_loss = test(net, testloader, loss_fn, device, scaler)
