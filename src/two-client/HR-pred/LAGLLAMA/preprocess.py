...
def time_to_seconds(time_str, day):
    try:
        h, m, s = map(float, time_str.split(":"))
        total_seconds = h * 3600 + m * 60 + s
        total_seconds += day * 86400  # Add 24 hours for each day
        return total_seconds
...
def process_with_Lag_llama(filepath: str):
    try:
        ...
        logging.info("Preprocessing dataset for lag-llama...")
        df_rr = pd.read_csv(filepath)
        # Convert timestamp to seconds
        df_rr["timestamp"] = [time_to_seconds(x, y) for x, y in zip(df_rr["time"], df_rr["day"])]

        # Filter unrealistic ibi_s values
        df_rr["ibi_s"] = df_rr["ibi_s"].where(df_rr["ibi_s"].between(0.3, 2))
        df_rr["hr"] = 60 / df_rr["ibi_s"]

        # Drop NaNs and remove duplicates
        df_rr.dropna(inplace=True)
        df_rr.drop_duplicates(subset=['timestamp'], inplace=True)

        # Convert timestamp to datetime and set as index
        df_rr["timestamp"] = pd.to_datetime(df_rr["timestamp"], unit='s')
        df_rr.set_index("timestamp", inplace=True)

        # Ensure 'hr' column is numeric
        df_rr["hr"] = pd.to_numeric(df_rr["hr"], errors='coerce')

        # Keep only 'hr' and 'timestamp' columns
        df_rr = df_rr[['hr']]

        # Resample to a uniform frequency (e.g., 1 second) and interpolate missing values
        df_rr = df_rr.resample('1S').mean()  # Resample to 1-second intervals
        df_rr = df_rr.interpolate('linear')  # Interpolate missing values
...
def SmartcareProcess_with_Lag_llama(filepath: str):
    try:
